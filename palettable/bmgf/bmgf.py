...
__all__ = ('COLOR_MAPS', 'print_maps', 'print_all_maps', 'print_maps_by_type',
           'get_map', 'MAP_TYPES', 'BMGFMap')

f = resource_string(__name__, 'data/bmgf_all_schemes.json')
...
class BMGFMap(Palette):
    """
    Representation of a BMGF color map with matplotlib compatible
    views of the map. Based off of colorbrewer class.
...
def get_map(name, map_type, number, reverse=False):
    """
    Return a `BMGFMap` representation of the specified color map.
    ...
    return BMGFMap(name, map_type.lower(), colors)
...
def _load_maps_by_type(map_type):
    ...
    Color maps are loaded as BMGFMap objects. Dictionary is
